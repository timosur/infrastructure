locals {
  ...
  ssh_public_key = var.ssh_public_key != "" ? var.ssh_public_key : "~/.ssh/id_ed25519.pub"
  ssh_private_key = var.ssh_private_key != "" ? var.ssh_private_key : "~/.ssh/id_ed25519"
}
...
module "kube-hetzner" {
  ...
  # Transmit extra arguments to the Cluster Autoscaler binary
  # These arguments are added to those generated by the preceding options
  # You should not set the following arguments because they are managed by terraform-hcloud-kube-hetzner:
  #   - --v=1
  #   - --cloud-provider=hetzner
  #   - --stderrthreshold=info
  #   - --nodes (provided multiple times if you setup multiple autoscaler_nodepools)
  # See the Cluster Autoscaler FAQ for the arguments list: https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/FAQ.md#what-are-the-parameters-to-ca
  # These arguments are not used if autoscaler_nodepools is not set, because the Cluster Autoscaler is installed only if autoscaler_nodepools is set
  ...
  extra_firewall_rules = [
    {
      description = "Allow Outbound TCP SMB 445"
      direction       = "out"
      protocol        = "tcp"
      port            = "445"
